...
export default {
  ...
  methods: {
    async getDayAmount(){
      let res = await financeApi.gatDayAmount()
    ...
    async getData() {
      let res = await financeApi.getDayRentalData()
      if (res.success && res.data != null) {
        this.rentalHours = res.data[0]
        this.rentalCount = res.data[1]
        this.updateRentalChart()
      }
      let resReturn = await financeApi.getDayReturnData()
      if (resReturn.success && resReturn.data != null) {
        this.returnHours = resReturn.data[0]
